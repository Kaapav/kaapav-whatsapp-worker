...
// defensive setSocket call — avoids crash and prints diagnostics
let buttonHandlerModule;
try {
  // require the module (same path you already use)
  buttonHandlerModule = require('./utils/buttonHandler');
  // helpful debugging info — prints what was exported and where resolved from
  try {
    const resolved = require.resolve('./utils/buttonHandler');
    console.log('buttonHandler resolved to:', resolved);
    console.log('buttonHandler cached filename:', require.cache[resolved]?.filename);
  } catch (e) { /* ignore resolve errors */ }

  console.log('buttonHandler exports keys:', Object.keys(buttonHandlerModule || {}));
} catch (e) {
  console.error('Failed to require ./utils/buttonHandler:', e && e.stack ? e.stack : e);
}

// call if valid
if (buttonHandlerModule && typeof buttonHandlerModule.setSocket === 'function') {
  try {
    buttonHandlerModule.setSocket(io);
    console.log('setSocket successfully called.');
  } catch (err) {
    console.error('Error while calling setSocket:', err && err.stack ? err.stack : err);
  }
} else {
  console.error('setSocket is missing or not a function. Exported keys:', Object.keys(buttonHandlerModule || {}));
  // fallback: continue without socket wiring to avoid process crash
}
...
app.post('/webhooks/whatsapp/cloudapi', async (req, res) => {
  ...
  try {
    ...
    // skip echoes of our own number
    if (WA_PHONE_ID && value?.metadata?.phone_number_id && String(value.metadata.phone_number_id) === String(WA_PHONE_ID)) return;
